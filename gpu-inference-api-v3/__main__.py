            ...
            [
                {
                    ...
                    "command": [
                        "sh",
                        "-c",
                        'pip install fastapi uvicorn && python -c \'from fastapi import FastAPI; import uvicorn; app = FastAPI(); @app.get("/health"): async def health(): return {"status": "healthy"}; uvicorn.run(app, host="0.0.0.0", port=8080)\'',
                    ],
                    "logConfiguration": {
                        "logDriver": "awslogs",
                        "options": {
